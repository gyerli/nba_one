...
SELECT ROW_NUMBER() over (partition BY season, pos order by player) gr,
  ...
  RANK() OVER (PARTITION BY season, pos ORDER BY a.dr_pts_rank + a.dr_pie_rank + a.dr_min_rank + a.dr_usg_pct_rank + a.dr_tov_rank + dr_a.reb_rank + a.dr_ast_rank + a.sdr_tl_rank + a.dr_blk_rank ) dr_ictsh_rank,
  a.loc,
  a.team,
  a.opponent,
  a.salary,
...
FROM
  (SELECT 
    ...
    rank() over (partition BY ps.season, fp.position order by COALESCE(pr.pts, po.pts)     * 0.8715039 DESC) dr_pts_rank,
    rank() over (partition BY ps.season, fp.position order by COALESCE(pra.pie, pa.pie)     * 0.7958694 DESC) dr_pie_rank,
    rank() over (partition BY ps.season, fp.position order by COALESCE(pr.min, po.min)     * 0.7457481 DESC) dr_min_rank,
    rank() over (partition BY ps.season, fp.position order by COALESCE(pra.usg_pct, pa.usg_pct) * 0.7006278 DESC) dr_usg_pct_rank,
    rank() over (partition BY ps.season, fp.position order by COALESCE(pr.tov, po.tov)     * 0.6086654 ASC) dr_tov_rank,
    rank() over (partition BY ps.season, fp.position order by COALESCE(pr.reb, po.reb)     * 0.5533486 DESC) dr_reb_rank,
    rank() over (partition BY ps.season, fp.position order by COALESCE(pr.ast, po.ast)     * 0.4750345 DESC) dr_ast_rank,
    rank() over (partition BY ps.season, fp.position order by COALESCE(pr.stl, po.stl)     * 0.4744307 DESC) dr_stl_rank,
    rank() over (partition BY ps.season, fp.position order by COALESCE(pr.blk, po.blk)     * 0.3305021 DESC) dr_blk_rank,
