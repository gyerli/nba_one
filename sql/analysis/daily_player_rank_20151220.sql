...
case when b.injury = 'O' then 99 else dense_rank() over (partition by b.fd_pos order by b.min_season + coalesce(b.inj_dist/60,0) desc) end min_pot_inj_rank,
...
round((rpt.get_weekly_minunte_dist_from_inj(a.season, a.team_abbrv, a.player_id, a.game_date, true)/60)::numeric,2) inj_dist,
a.min_season,
a.fdpts_season,
a.usg_pct_season,
a.pie_season,
...
coalesce(fp.salary, fpx.salary) salary,
coalesce(fp.fppg, fpx.fppg) fppg,
coalesce(fp.injury_indicator, fpx.injury_indicator) AS injury,
coalesce(fp.injury_details, fpx.injury_details) AS injury_det,
...
left join stg.xlat_player xp on (p.id = xp.player_id )
